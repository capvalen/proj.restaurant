...
while($row = mysqli_fetch_array($sql, MYSQLI_ASSOC))
{
	...
	if($numCombos>0){
		...
		echo "<h4 class='col-xs-9' style='padding-left:58px;'><span class='h4NombreProducto'>".ucwords($row['prodDescripcion'])."</span> x <span class='cantidadProducto'>".$numCombos."</span> Combo de ".$row['promoCantidad']." unds. </h4><h5 class='col-xs-3 dscPacial'> S/ -<span class='montoDescontar'>". number_format($numCombos*$row['promoDescuento'],2)."</span></h5>";
		echo "</div>";
	}
